...
async function carregarAlunos2() {
    ...
    try {
        const querySnapshot = await getDocs(collection(db, "turma8a")); // Obtém os documentos
...
async function atualizarPontuacao(pontosAdicionais) {
    ...
    try {
        ...
        querySnapshot.forEach(async (documento) => {
            const alunoRef = doc(db, "turma8a", documento.id);
...
async function carregarAlunos() {
    ...
    try {
        const querySnapshot = await getDocs(collection(db, "turma8a"));
        // Armazena os alunos em um array
        const alunos = [];
        querySnapshot.forEach((doc) => {
